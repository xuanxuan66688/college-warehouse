...
int8_t Tracking(void)
{
	...
	&&HAL_GPIO_ReadPin(GPIOC,TRA5_Pin)==1
	&&HAL_GPIO_ReadPin(GPIOC,TRA6_Pin)==1)//全黑到十字路口
	  {
		  return 1;
	  }
	  ...
	  &&HAL_GPIO_ReadPin(GPIOC,TRA5_Pin)==1)
	  {
		  ...
		  &&HAL_GPIO_ReadPin(GPIOC,TRA4_Pin)==1
	  &&HAL_GPIO_ReadPin(GPIOC,TRA6_Pin)==1)
	  {
		 return 3;
	  }
	  ...
	  else if (HAL_GPIO_ReadPin(GPIOC,TRA5_Pin)==1)
	  {
		  set_speed(-30,0);
		  set_speed(-15,0);
		  //printf("5\r\n");
	  }
	else if (HAL_GPIO_ReadPin(GPIOC,TRA6_Pin)==1)
	  {
		  set_speed(-15,0);
		   set_speed(-30,1);
		  //printf("6\r\n");
